...
type AWSCodeCommitConnection struct {
	...
	// RepositoryPathPattern description: The pattern used to generate a the corresponding Nxpkg repository path for an AWS CodeCommit repository. In the pattern, the variable "{name}" is replaced with the repository's name.
	//
...
type BitbucketServerConnection struct {
	...
	// RepositoryPathPattern description: The pattern used to generate the corresponding Nxpkg repository path for a Bitbucket Server repository.
	//
...
type ExperimentalFeatures struct {
	...
	// JumpToDefOSSIndex description: Enables or disables consulting the OSS package index on Nxpkg.com for cross repository jump to definition. When enabled Nxpkg.com will receive Code Intelligence requests when they fail to resolve locally. NOTE: disablePublicRepoRedirects must not be set, or should be set to false.
...
type GitHubConnection struct {
	...
	// Repos description: An array of repository "owner/name" strings specifying which GitHub or GitHub Enterprise repositories to mirror on Nxpkg.
	Repos []string `json:"repos,omitempty"`
	// RepositoryPathPattern description: The pattern used to generate a the corresponding Nxpkg repository path for a GitHub or GitHub Enterprise repository. In the pattern, the variable "{host}" is replaced with the GitHub host (such as github.example.com), and "{nameWithOwner}" is replaced with the GitHub repository's "owner/path" (such as "myorg/myrepo").
	//
	...
	RepositoryPathPattern string `json:"repositoryPathPattern,omitempty"`
	// RepositoryQuery description: An array of strings specifying which GitHub or GitHub Enterprise repositories to mirror on Nxpkg. The valid values are:
	//
...
type GitLabConnection struct {
	...
	GitURLType string `json:"gitURLType,omitempty"`
	// InitialRepositoryEnablement description: Defines whether repositories from this GitLab instance should be enabled and cloned when they are first seen by Nxpkg. If false, the site admin must explicitly enable GitLab repositories (in the site admin area) to clone them and make them searchable on Nxpkg. If true, they will be enabled and cloned immediately (subject to rate limiting by GitLab); site admins can still disable them explicitly, and they'll remain disabled.
	InitialRepositoryEnablement bool `json:"initialRepositoryEnablement,omitempty"`
	// ProjectQuery description: An array of strings specifying which GitLab projects to mirror on Nxpkg. Each string is a URL query string for the GitLab projects API, such as "?membership=true&search=foo".
	//
	...
	// RepositoryPathPattern description: The pattern used to generate a the corresponding Nxpkg repository path for a GitLab project. In the pattern, the variable "{host}" is replaced with the GitLab URL's host (such as gitlab.example.com), and "{pathWithNamespace}" is replaced with the GitLab project's "namespace/path" (such as "myteam/myproject").
	//
...
type Langservers struct {
	// Address description: TCP address of the language server. Required for Nxpkg Server; do not set for Nxpkg Data Center.
...
type OpenIDConnectAuthProvider struct {
	...
	// RequireEmailDomain description: Only allow users to authenticate if their email domain is equal to this value (example: mycompany.com). Do not include a leading "@". If not set, all users on this OpenID Connect provider can authenticate to Nxpkg.
...
type Repository struct {
	...
	// Path description: Display path on Nxpkg for the repository, such as my/repo
	Path string `json:"path"`
...
type SiteConfiguration struct {
	...
	// AuthDisableAccessTokens description: Prevents users from creating access tokens, which enable external tools to access the Nxpkg API with the privileges of the user.
	...
	// DisableBrowserExtension description: Disable incoming connections from the Nxpkg browser extension.
	...
	// DisablePublicRepoRedirects description: Disable redirects to nxpkg.com when visiting public repositories that can't exist on this server.
	DisablePublicRepoRedirects bool `json:"disablePublicRepoRedirects,omitempty"`
	// DisableTelemetry description: (Deprecated: event-level telemetry is now always disabled) Prevent usage data from being sent back to Nxpkg (no private code is sent and URLs are sanitized to prevent leakage of private data).
	...
	// HttpStrictTransportSecurity description: The value of the Strict-Transport-Security HTTP header sent by Nxpkg, if non-empty
	...
	// SiteID description: The identifier for this site. A Nxpkg site is a collection of one or more Nxpkg instances that are all part of the same logical site. If the site ID is not set here, it is stored in the database the first time the server is run.
	...
	// UpdateChannel description: The channel on which to automatically check for Nxpkg updates.
